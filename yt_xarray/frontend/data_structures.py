...
from .fields import yt_xarrayFieldInfo


class yt_xarrayGrid(AMRGridPatch):
    _id_offset = 0
    ...
    def __repr__(self):
        return "yt_xarrayGrid_%04i (%s)" % (self.id, self.ActiveDimensions)


class yt_xarrayHierarchy(GridIndex):
    grid = yt_xarrayGrid

    def __init__(self, ds, dataset_type="yt_xarray"):
...
class yt_xarrayDataset(Dataset):
    _index_class = yt_xarrayHierarchy
    _field_info_class = yt_xarrayFieldInfo

    def __init__(
        self,
        filename,
        dataset_type="yt_xarray",
        storage_filename=None,
        units_override=None,
        unit_system="cgs",
        default_species_fields=None,
    ):
        self.fluid_types += ("yt_xarray",)
        super().__init__(
            filename,
            dataset_type,
